...
(function(){
  ...
  function 主資料庫(){return 設定.SPREADSHEET_ID||設定.正式主資料庫ID||'19osmTlQQ9obDmVvmv5uphFHRwCtd2pkFhe6p3pYMSn8'}
  function 補庫(p){p=Object.assign({},p||{});p.spreadsheetId=p.spreadsheetId||主資料庫();p.正式主資料庫ID=p.正式主資料庫ID||主資料庫();p.主資料庫ID=p.主資料庫ID||主資料庫();return p}
  function buildURL(action,payload){var u=new URL(baseURL());u.searchParams.set('action',action);u.searchParams.set('動作',action);u.searchParams.set('_ts',Date.now());Object.keys(補庫(payload)).forEach(function(k){var x=補庫(payload)[k];if(x==null)return;u.searchParams.set(k,typeof x==='object'?JSON.stringify(x):String(x))});return u.toString()}
  function body(action,payload){var sp=new URLSearchParams();var obj=Object.assign({},補庫(payload),{action:action,動作:action,_ts:String(Date.now())});Object.keys(obj).forEach(function(k){var x=obj[k];if(x==null)return;sp.set(k,typeof x==='object'?JSON.stringify(x):String(x))});var json=JSON.stringify(obj);if(json.length<220000){sp.set('payload',json);sp.set('資料',json);sp.set('json',json)}return sp.toString()}
