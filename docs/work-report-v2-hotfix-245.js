(function(){
  ...
  function injectSubmitEffect(){if(document.getElementById('確認送出起浮效果'))return;var s=document.createElement('style');s.id='確認送出起浮效果';s.textContent='#送出報工.送出起浮{transform:translateY(-4px);box-shadow:0 18px 34px rgba(0,178,255,.34),0 8px 18px rgba(16,185,129,.22),inset 0 1px 0 rgba(255,255,255,.28);transition:transform .16s ease,box-shadow .16s ease,filter .16s ease;will-change:transform;position:relative;}#送出報工.送出起浮:active{transform:translateY(1px) scale(.985);box-shadow:0 7px 14px rgba(0,178,255,.22),inset 0 2px 6px rgba(0,0,0,.22);filter:brightness(.96);}#送出報工.送出起浮::after{content:"";position:absolute;inset:2px;border-radius:inherit;pointer-events:none;background:linear-gradient(180deg,rgba(255,255,255,.20),rgba(255,255,255,0) 38%);opacity:.75;}@media (prefers-reduced-motion:reduce){#送出報工.送出起浮{transition:none;}}';document.head.appendChild(s)}
  function isQuality(){var q=document.querySelector('[data-tab="品質"]');return !!(q&&q.classList.contains('active'))}
  function button(){var b=$('送出報工');if(!b)return;if(isQuality()){wtxt(b,'確認送出 / Confirm Submit');b.classList.add('送出起浮');}else{wtxt(b,'下一步 / Next →');b.classList.remove('送出起浮')}}
  ...
  function run(){bind();injectSubmitEffect();labels();overtime();timeFix();defect();button()}
