...
async function submitReport(){try{var d={來源:'07_報工作業v2'};['工號','姓名','班別','區域','機台編號','產品編號','品名','工站名稱','工單編號','良品數','不良數','停機分鐘','停機原因','換刀次數','備註'].forEach(function(k){d[k]=v(k)});if(!d.工號&&!d.姓名)throw new Error('請填工號或姓名');if(!d.產品編號)throw new Error('請填產品編號');if(!d.工站名稱)throw new Error('請填工站名稱');if(!d.機台編號)throw new Error('請填機台編號');if(Number(d.良品數||0)+Number(d.不良數||0)<=0)throw new Error('良品數+不良數必須大於0');out('送出中...');out(JSON.stringify(await api('新增報工',d),null,2))}catch(e){out('錯誤：'+e.message)}}
window.addEventListener('load',function(){setv('GAS網址',localStorage.getItem('GAS網址')||'');['姓名','工號'].forEach(function(id){document.getElementById(id).addEventListener('change',autoPerson)});document.getElementById('產品編號').addEventListener('change',autoProduct);document.getElementById('工站名稱').addEventListener('change',autoStation);document.getElementById('機台編號').addEventListener('change',autoMachine)});