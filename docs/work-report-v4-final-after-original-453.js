/* 製一組報工表單 V4｜後置產品工站修復 v4.5.3
 * 位置：原 work-report-v4.html 內建 JS 完全載入後。
 * 只修：點產品後帶入產品資料、原工站 select 自動展開、選工站後顯示機台。
 * 重點：原始 DB / STATE 是 let，不是 window.DB；本版改用同一全域 lexical binding，不再用 window.DB 判斷。
 */
(function(){
  'use strict';
  var VER='v4.5.3_after_original_station_binding_lexical_DB';
  ...
  function scroll(el){if(!el)return;setTimeout(function(){try{el.scrollIntoView({behavior:'smooth',block:'center'});}catch(e){try{el.scrollIntoView();}catch(_){}}},80);}
  function 資料已存在(){try{return typeof DB!=='undefined'&&typeof STATE!=='undefined'&&DB&&STATE;}catch(e){return false;}}
  function hideOld(){['v4StationDock444','v445StationPanel','v4StationQuickPicker443','v4StationQuickPicker','v447StationNotice','v448StationPanel','v449StationHint','v450StationHint','v451StationHint','v452hint'].forEach(function(id){var e=$(id);if(e)e.remove();});}
  function style(){
    if($('v453style'))return;
    var s=document.createElement('style');s.id='v453style';
    s.textContent=[
      '#v4StationDock444,#v445StationPanel,#v4StationQuickPicker443,#v4StationQuickPicker,#v447StationNotice,#v448StationPanel,#v449StationHint,#v450StationHint,#v451StationHint,#v452hint{display:none!important}',
      ...
      '#workstationSelect.v453open{height:auto!important;min-height:170px!important}',
      '#v453hint{margin:10px 0;padding:11px 12px;border-radius:16px;background:#fff8d7;border:2px solid rgba(251,188,4,.6);font-weight:900;color:#6b4a00;line-height:1.45;font-size:13px}',
      '.v453focus{animation:v453pulse 1.1s ease-in-out 2;box-shadow:0 0 0 7px rgba(66,133,244,.20)!important}',
      '@keyframes v453pulse{0%,100%{transform:scale(1)}50%{transform:scale(1.018)}}'
    ].join('\n');
    document.head.appendChild(s);
  }
  function focus(el){if(!el)return;el.classList.add('v453focus');scroll(el);setTimeout(function(){try{el.classList.remove('v453focus');}catch(e){}},2200);}
  ...
  function fillStation(routes,product){
    ...
    s.value='';s.onchange=chooseStation;
    if(routes.length){s.size=Math.min(routes.length+1,6);s.classList.add('v453open');}else{s.size=1;s.classList.remove('v453open');}
    var old=$('v453hint');if(old)old.remove();
    var hint=document.createElement('div');hint.id='v453hint';
  ...
  function selectProduct453(key,card){
    style();hideOld();
    if(!資料已存在())return;
    if(!key&&card)key=cardKey(card);
    var product=findProduct(key);var routes=routesFor(product,key);
    STATE.productGroupList=routes;STATE.currentProductGroup=routes[0]||product;STATE.currentWorkstation=null;
  ...
  function chooseStation(){
    ...
    s.size=1;s.classList.remove('v453open');
  ...
  function install(){
    style();hideOld();
    window.selectProduct=function(key){selectProduct453(key,null);};
    window.onWorkstationChange=chooseStation;
    window.V4_453_選產品=selectProduct453;
  }
  function handler(ev){var card=ev.target&&ev.target.closest&&ev.target.closest('.product-card');if(!card)return;ev.preventDefault();ev.stopPropagation();if(ev.stopImmediatePropagation)ev.stopImmediatePropagation();selectProduct453(cardKey(card),card);}
  document.addEventListener('click',handler,true);
  document.addEventListener('touchstart',function(ev){var c=ev.target&&ev.target.closest&&ev.target.closest('.product-card');if(c)window.__v453_touch_card=c;},true);
  document.addEventListener('touchend',function(ev){var c=ev.target&&ev.target.closest&&ev.target.closest('.product-card');if(!c)c=window.__v453_touch_card;if(c){ev.preventDefault();ev.stopPropagation();if(ev.stopImmediatePropagation)ev.stopImmediatePropagation();selectProduct453(cardKey(c),c);window.__v453_touch_card=null;}},true);
