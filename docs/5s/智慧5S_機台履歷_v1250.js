(function (全域) {
  ...
  function 圖片來源(row){const src=第一值(row,['照片資料','照片網址','圖片網址']);return /^(data:image\/|https?:\/\/)/i.test(src)?src:'';}
  ...
  function 注入樣式(){
    ...
    const s=document.createElement('style');s.id='機台履歷1250樣式';s.textContent=`
      ...
      .MH照片網格{display:grid;grid-template-columns:repeat(3,minmax(0,1fr));gap:8px;margin-top:10px}.MH照片卡{min-width:0;background:#f6f9f7;border:1px solid #e0e8e3;border-radius:14px;overflow:hidden}.MH照片卡 img{display:block;width:100%;aspect-ratio:4/3;object-fit:cover;background:#e9efeb}.MH照片卡 div{padding:7px}.MH照片卡 b{display:block;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;font-size:.67rem}.MH照片卡 small{display:block;color:#7c8c83;font-size:.58rem;margin-top:3px;line-height:1.35}.MH照片無圖{aspect-ratio:4/3;display:grid;place-items:center;background:#edf2ef;color:#72857a;font-size:.65rem;text-align:center;padding:6px}
      .MH載入{display:grid;place-items:center;min-height:45vh;color:#5d7469;text-align:center}.MH載入 b{display:block;margin-top:10px}
      @media(max-width:520px){.MH摘要{grid-template-columns:repeat(2,minmax(0,1fr))}.MH面板{padding-left:10px;padding-right:10px}.MH頭{margin-left:-10px;margin-right:-10px;padding-left:10px;padding-right:10px}.MH責任{grid-template-columns:1fr}.MH數卡 b{font-size:1.35rem}.MH照片網格{grid-template-columns:repeat(2,minmax(0,1fr))}}
  ...
  function 照片HTML(photos){
    if(!photos.length)return'<div class="MH空">目前沒有與此機台關聯的照片證據</div>';
    return `<div class="MH照片網格">${photos.slice(0,6).map(r=>{const src=圖片來源(r);const no=第一值(r,['照片編號','參照單號'])||'照片';const type=第一值(r,['參照類型','資料摘要'])||'5S照片';const time=第一值(r,['拍攝時間','建立時間']).slice(0,16);return `<div class="MH照片卡">${src?`<img src="${轉義(src)}" loading="lazy" alt="${轉義(no)}">`:'<div class="MH照片無圖">照片已建檔<br>目前無可預覽來源</div>'}<div><b>${轉義(type)}</b><small>${轉義(no)}${time?`｜${轉義(time)}`:''}</small></div></div>`;}).join('')}</div>`;
  }
