(function (全域) {
  ...
  function 注入樣式() {
    ...
    樣式.id = 樣式識別;
    樣式.textContent = `
      .文件5S區{display:grid;gap:12px;margin-top:14px}.文件5S英雄{background:linear-gradient(135deg,#0f5135,#198a5c 63%,#7a244a);color:#fff;border-radius:22px;padding:16px;box-shadow:0 10px 28px rgba(18,73,50,.13)}.文件5S英雄 h3{margin:0 0 5px;font-size:1.08rem}.文件5S英雄 p{margin:0;line-height:1.55;color:rgba(255,255,255,.86);font-size:.78rem}.文件5S英雄 small{display:block;margin-top:8px;color:rgba(255,255,255,.72);font-weight:800}
      ...
      .文件5S篩選{display:flex;gap:7px;overflow-x:auto;scrollbar-width:none;padding:2px 0}.文件5S篩選::-webkit-scrollbar{display:none}.文件5S篩選 button{flex:0 0 auto;min-height:42px;border:1px solid #dce7e1;border-radius:999px;background:#fff;color:#365345;padding:8px 13px;font-weight:900}.文件5S篩選 button.作用中{background:#176b47;color:#fff;border-color:#176b47}
      ...
      .文件5S照片雙欄{display:grid;grid-template-columns:1fr 1fr;gap:1px;background:#e5ebe7}.文件5S照片單欄{display:grid;grid-template-columns:1fr}.文件5S圖框{position:relative;background:#edf2ef;aspect-ratio:4/3;overflow:hidden;cursor:zoom-in}.文件5S圖框 img{width:100%;height:100%;display:block;object-fit:cover}.文件5S圖標{position:absolute;left:7px;top:7px;border-radius:999px;padding:4px 7px;background:rgba(17,53,38,.82);color:#fff;font-size:.64rem;font-weight:900}.文件5S圖標.前{background:rgba(135,55,45,.86)}
      ...
      .文件5S燈箱{position:fixed;inset:0;z-index:99999;background:rgba(10,22,16,.92);display:flex;align-items:center;justify-content:center;padding:16px}.文件5S燈箱.隱藏{display:none}.文件5S燈箱 img{max-width:96vw;max-height:84vh;border-radius:16px;object-fit:contain;background:#fff}.文件5S燈箱 button{position:absolute;right:16px;top:max(16px,env(safe-area-inset-top));width:48px;height:48px;border:0;border-radius:50%;background:#fff;color:#173f2f;font-size:26px;font-weight:900}
  ...
  function 圖片(來源, 標籤, 類別) {
    const src = 文字(來源);
    if (!src) return '';
    return `<div class="文件5S圖框" data-文件照片="${轉義(src)}"><img src="${轉義(src)}" alt="${轉義(標籤)}" loading="lazy"><span class="文件5S圖標 ${類別 || ''}">${轉義(標籤)}</span></div>`;
  }
  ...
  function 卡片HTML(列) {
    ...
    let 圖片區 = '';
    if (有改善前 && 有改善後) {
      圖片區 = `<div class="文件5S照片雙欄">${圖片(列.錯誤照片, '改善前', '前')}${圖片(列.正確照片, '改善後', '')}</div>`;
    } else if (有改善後) {
      圖片區 = `<div class="文件5S照片單欄">${圖片(列.正確照片, '標準照片', '')}</div>`;
    } else if (有改善前) {
      圖片區 = `<div class="文件5S照片單欄">${圖片(列.錯誤照片, '現況照片', '前')}</div>`;
    }

    const 錯誤說明 = 文字(列.錯誤說明);
    const 正確做法 = 文字(列.正確做法);
    return `<article class="文件5S卡" data-文件群組="${轉義(區域群組(列))}">
      <div class="文件5S卡頭"><div><h4>${轉義(列.問題類型 || '5S標準')}</h4><p>${轉義(列.區域名稱 || 列.區域代碼)}｜${轉義(列.子區域代碼 || '')}</p></div><span class="文件5S頁碼">P${String(頁碼).padStart(2,'0')}</span></div>
      ${圖片區}
      <div class="文件5S內容"><div class="文件5S說明">
        ${錯誤說明 ? `<div><b>現況／問題：</b>${轉義(錯誤說明)}</div>` : ''}
        ${正確做法 ? `<div><b>標準／做法：</b>${轉義(正確做法)}</div>` : ''}
      </div>
      <div class="文件5S追蹤">${轉義(列.適用生產狀態 || '9月追蹤：待補改善資料')}</div>
      <div class="文件5S來源">來源 ${轉義(列.版本 || 'FM-3-001-02')}｜更新 ${轉義(列.更新時間 || '')}</div></div>
    </article>`;
  }
  ...
  function 總覽HTML(清單) {
    ...
    return `<section id="${注入識別}" class="文件5S區">
      <div class="文件5S英雄"><h3>📘 日常點檢改善｜FM-3-001-02</h3><p>已把文件中的改善前後、檢驗桌擺放標準與掃具櫃標準接入智慧5S可視化資料。</p><small>文件整合模組 v${模組版本}｜系統 v${系統版本}</small></div>
      <div class="文件5SKPI"><article><small>文件標準／改善</small><b>${清單.length}</b><span>P02–P18</span></article><article><small>A9</small><b>${s.A9}</b><span>1069／1070檢驗桌</span></article><article><small>A5</small><b>${s.A5}</b><span>1044／1045／1046</span></article><article><small>9月待追蹤</small><b>${s.待追蹤}</b><span>需提供改善資料</span></article></div>
    </section>`;
  }

  function 標準頁HTML(清單) {
    const 群組 = ['全部','A9','A5','B9','B3','掃具'];
    const 篩選後 = 狀態.篩選 === '全部' ? 清單 : 清單.filter(列 => 區域群組(列) === 狀態.篩選);
    return `<section id="${注入識別}" class="文件5S區">
      <div class="文件5S英雄"><h3>📷 日常點檢改善標準照片</h3><p>文件 FM-3-001-02 共 ${清單.length} 筆。改善前後對照與單張標準照片分開呈現，可直接作為現場確認基準。</p><small>9月追蹤：2026-09-01 與茂軒溝通，需提供改善資料</small></div>
      <div class="文件5S篩選">${群組.map(名稱 => `<button type="button" data-文件篩選="${名稱}" class="${狀態.篩選===名稱?'作用中':''}">${名稱}</button>`).join('')}</div>
  ...
  function 開啟燈箱(src) {
    ...
    if (!燈箱) {
      ...
      燈箱.innerHTML = '<button type="button" aria-label="關閉照片">×</button><img alt="5S標準照片">';
      document.body.appendChild(燈箱);
      燈箱.querySelector('button').onclick = () => 燈箱.classList.add('隱藏');
      燈箱.onclick = 事件 => { if (事件.target === 燈箱) 燈箱.classList.add('隱藏'); };
    }
    燈箱.querySelector('img').src = src;
  ...
  function 綁定事件() {
    ...
    document.querySelectorAll('#' + 注入識別 + ' [data-文件照片]').forEach(框 => {
      框.onclick = () => 開啟燈箱(文字(框.getAttribute('data-文件照片')));
    });
  }

  function 排程注入() {
    clearTimeout(狀態.計時器);
    狀態.計時器 = setTimeout(() => 注入().catch(錯誤 => console.warn('日常點檢改善注入失敗', 錯誤)), 90);
  }
  ...
  function 綁定觀察() {
    ...
    狀態.觀察器.observe(容器, { childList:true, subtree:true, attributes:true, attributeFilter:['class'] });
