(function (全域) {
  ...
  function 從目前畫面取得紅牌(id){
    ...
    return {
      紅牌編號:id,掛牌日:'',盤點編號:'',部門:'製造部',區域:info[0]||areaFromModal||'',
      物品名稱:文字(card.querySelector('h2')?.textContent)||'未填',規格型號:'—',數量:qty[1]||'',單位:qty[2]||'',
      ...
      責任部門:'製造部',責任人:'待指派',預定處置日:(tags.find(x=>x.startsWith('期限：'))||'').replace(/^期限：/,''),
      案件狀態:tags.find(x=>/待處置|處理中|待複查|已結案|已完成/.test(x))||'待處置',_畫面備援:true
    };
  }
  ...
  async function 取得掛牌序號(red){
    ...
    const prefix=`RP-${日期緊縮(red.掛牌日)}-`;let max=0;
  ...
  async function 記錄列印(red,serial,count){
    const u=目前使用者();
    const data={列印紀錄編號:識別碼('5S-PRT'),紅牌編號:文字(red.紅牌編號),掛牌序號:serial,掛牌日:文字(red.掛牌日),列印時間:完整時間(),列印人工號:文字(u.工號),列印人姓名:文字(u.姓名),列印版型:'A5直式含QR',列印用途:'現場實體掛牌',備註:`第${count}次列印｜智慧5S v${版本}${red._畫面備援?'｜中央主檔查無資料，使用畫面備援':''}`};
  ...
  function 列印HTML(red,serial,count){
    const back=系統網址('紅牌');const qr=`https://quickchart.io/qr?size=220&margin=1&ecLevel=M&text=${encodeURIComponent(掃碼網址(red.紅牌編號,serial))}`;
  ...
  document.addEventListener('click',e=>{
    const b=e.target.closest('[data-rp-action="print"],#列印現場紅牌');if(!b)return;
    const id=文字(b.dataset.rpId)||目前紅牌編號();if(!id)return;
    e.preventDefault();e.stopPropagation();e.stopImmediatePropagation();執行新版列印(id);
  },true);
