/* 報工作業 V4 submit v4.8.9：寫入 09_報工 / 09_不良紀錄
 * 鎖定正式主資料庫；機台清單固定吃 08_工站途程機台主檔；避免前端誤判未確認。
 * 加入選定人員加班欄位：是否加班、加班類型、加班小時。
 */
(function(){
'use strict';
...
function readOvertime(){
  if(window.V4Overtime489&&typeof window.V4Overtime489.read==='function')return window.V4Overtime489.read();
  const f=S(E('overtimeFlag')?.value||'否');
  const t=S(E('overtimeType')?.value||'無');
  const h=f==='是'?S(E('overtimeHours')?.value):'';
  return {是否加班:f||'否',加班類型:f==='是'?(t||'平日加班'):'無',加班小時:h};
}
...
function buildPayload(){
  ...
  return {reportId,報工編號:reportId,workDate:window.V4Bridge?.today?.()||'',作業日:window.V4Bridge?.today?.()||'',是否加班:overtime.是否加班,加班類型:overtime.加班類型,加班小時:overtime.加班小時,operator:{employeeId:S(op.工號||E('personId')?.value),operatorName:S(op.姓名||E('personName')?.value),shift:S(op.班別||E('shift')?.value||'早班'),overtime:overtime.是否加班,overtimeType:overtime.加班類型,overtimeHours:overtime.加班小時,是否加班:overtime.是否加班,加班類型:overtime.加班類型,加班小時:overtime.加班小時},product:{productCode:S(prod.產品編號||E('productCode')?.value),customerPartNo:S(prod.客戶品號||E('customerPartNo')?.value),productName:S(prod.品名||E('productName')?.value)},workstation:{workstationName:S(route.工站名稱||route.報工工站名稱||E('workstationSelect')?.selectedOptions?.[0]?.textContent||''),processRange:S(route.工序範圍||route.工序||E('processRange')?.value),machineList:machineListText(route),mainMachine:S(machine.機台編號||machine.主機台||E('mainMachineSelect')?.value)},output:{totalQty:total,ngQty:ng,goodQty:good,startTime:S(E('startTime')?.value),endTime:S(E('endTime')?.value),workingHours:S(E('workingHours')?.value||'8 hrs'),overtime:overtime.是否加班,overtimeType:overtime.加班類型,overtimeHours:overtime.加班小時},quality:{defects,defectSummary:defects.map(d=>`${d.分類||''}類 ${d.不良代碼} ${d.不良名稱} × ${d.不良數量} pcs`).join('；')},photos:window.V4_PHOTOS||[],remarks:S(E('remarks')?.value),source:'PWA_V4.8.9_LOCKED_UIUX_OVERTIME'};
}
