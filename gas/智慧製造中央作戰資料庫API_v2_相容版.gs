/**
 * 智慧製造中央作戰資料庫 API v2 相容版
 * 修正重點：
 * 1. 回傳同時包含 ok:true 與 success:true，支援舊前端/新前端。
 * 2. doGet 支援 action=初始化 / 自檢 / schema。
 * 3. doGet 支援 callback JSONP，避免前端 CORS 讀不到回應。
 * 4. doPost 支援 action、動作、method 三種欄位。
 * 5. 可直接整合到既有主程式的 doGet/doPost 路由。
 */
...
function 自檢_智慧製造中央作戰資料庫() {
  const ss = 取得資料庫_必要時建立_();
  const 分頁檢查 = Object.keys(資料表欄位).map(name => {
...
function 寫入資料庫快照_(body) {
  ...
  const 原始表 = 建立或校正分頁_(ss, '90_匯入原始資料', 資料表欄位['90_匯入原始資料']);
  原始表.appendRow([new Date(), 來源, '資料庫快照', JSON.stringify(payload).slice(0, 45000)]);
  更新BOM檢查清單_(ss);
  SpreadsheetApp.flush();
  return 成功_({ 訊息:'資料庫快照寫入完成', 批次ID, 資料庫ID:ss.getId(), 資料庫網址:ss.getUrl(), 寫入結果 });
}
...
function 清空指定分頁_(name) {
  if (!name || !資料表欄位[name]) return 失敗_('分頁不存在或不可清空：' + name);
  const ss = 取得資料庫_必要時建立_();
  const sh = 建立或校正分頁_(ss, name, 資料表欄位[name]);
  if (sh.getLastRow() > 1) sh.getRange(2, 1, sh.getLastRow()-1, sh.getLastColumn()).clearContent();
  return 成功_({ 訊息:'已清空：' + name });
}
...
function 寫入系統設定_(ss) {
  ...
  const settings = [
    ...
    ['版本', 'v2_相容版', 'BOM排程版資料庫結構', new Date()],
...
function 輸出JSON_(obj, callback) {
  const text = JSON.stringify(obj);
  if (callback) {
    return ContentService.createTextOutput(String(callback) + '(' + text + ');').setMimeType(ContentService.MimeType.JAVASCRIPT);
  }
