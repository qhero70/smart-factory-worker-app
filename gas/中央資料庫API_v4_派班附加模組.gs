/**
 * 智慧製造中央作戰資料庫 API v4｜自動派班附加模組
 * 加到目前 v3 APS 獨立版同一個 GAS 專案即可。
 * 若要讓 PWA 按鈕直接呼叫，doPost 裡需加入：
 * else if(action==='執行自動派班V1'||action==='runDispatch') r=執行自動派班V1_(body);
 */

function 執行自動派班V1_(body){
  const ss=取得資料庫_必要時建立_();
  建立或校正分頁_(ss,'16_自動派班結果',['派班ID','日期','班別','工號','姓名','工站','機台','工單號','產品編號','品名','工單類型','排程數量','預計開始','預計完成','技能等級','派班狀態','備註']);
  建立或校正分頁_(ss,'17_人員負荷表',['日期','班別','工號','姓名','派班筆數','負荷小時','可用小時','負荷率','狀態','備註']);
  建立或校正分頁_(ss,'18_派班警示',['時間戳','等級','日期','班別','工單號','產品編號','品名','警示類型','警示內容','處置建議','狀態']);
  ...
  排程.filter(r=>取(r,'工單類型')==='PROD' ||取(r,'工單類型')==='PACK'||取(r,'工單類型')==='LOAD').forEach(r=>{
  ...
  寫入紀錄_(ss,'DSP_'+Utilities.formatDate(new Date(),Session.getScriptTimeZone(),'yyyyMMdd_HHmmss'),'GAS_DISPATCH_V1','16_自動派班結果',派班.length,'完成','自動派班V1');
