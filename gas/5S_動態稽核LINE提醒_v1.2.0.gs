 ...
 * 2. PWA 入口版本不再寫死；每次推播前讀 5S_系統參數「PWA入口版本」。
 * 3. 正式入口網址可由 5S_系統參數「PWA正式入口網址」或 Script Property「智慧5S_PWA網址」控制。
 * 4. 到期/逾期依 5S_稽核週期自動判斷，同日同區域去重。
 * 5. 仍使用既有唯一 LINE Bot；Token 只讀 Script Properties。
 *
...
function 智慧5S_預覽精簡LINE提醒(payload) {
  ...
  var today = String(payload['日期'] || '').trim() || 智慧5S動態稽核_日期_(new Date());
  var rows = 智慧5S動態稽核_讀工作表_(sh).filter(function(r) {
    var d = 智慧5S動態稽核_日期_(r['下次稽核日']);
    return d && d <= today;
  });
  return {ok:true,版本:智慧5S動態稽核_版本_,筆數:rows.length,PWA:智慧5S動態稽核_取得PWA網址_(),訊息:智慧5S動態稽核_建立訊息_(today,rows)};
}
...
function 智慧5S_刪除每日稽核提醒觸發器() {
  ...
  ScriptApp.getProjectTriggers().forEach(function(t) {
    if (t.getHandlerFunction() === 智慧5S動態稽核_觸發函數_) {
...
function 智慧5S動態稽核_讀系統參數_() {
  ...
  try {
    ...
    智慧5S動態稽核_讀工作表_(sh).forEach(function(r) {
      var key = String(r['參數鍵'] || '').trim();
      if (key) out[key] = String(r['參數值'] == null ? '' : r['參數值']).trim();
    });
  } catch (e) {
    console.warn('讀取5S系統參數失敗：' + e);
  }
  return out;
}
...
function 智慧5S動態稽核_讀提醒小時_() {
  try {
    var p = 智慧5S動態稽核_讀系統參數_();
    var raw = p['動態稽核LINE提醒時間'] || p['每日巡檢首次提醒時間'] || '8:00';
...
function 智慧5S動態稽核_取得PWA網址_() {
  ...
  var raw = String(p['PWA正式入口網址'] || props.getProperty('智慧5S_PWA網址') || 智慧5S動態稽核_正式入口_).trim();
  var version = String(p['PWA入口版本'] || 智慧5S動態稽核_入口版本備援_).replace(/\D/g,'') || 智慧5S動態稽核_入口版本備援_;
  if (!raw) raw = 智慧5S動態稽核_正式入口_;
  raw = raw.replace(/([?&])v=\d+/g,'$1').replace(/([?&])頁面=[^&]*/g,'$1').replace(/([?&])來源=[^&]*/g,'$1');
  raw = raw.replace(/\?&/g,'?').replace(/&&+/g,'&').replace(/[?&]+$/,'');
  var sep = raw.indexOf('?') >= 0 ? '&' : '?';
  return raw + sep + '頁面=巡檢&來源=LINE&v=' + version;
}
...
function 智慧5S動態稽核_推播_(token, groupId, message) {
  var res = UrlFetchApp.fetch('https://api.line.me/v2/bot/message/push', {
    ...
    payload:JSON.stringify({to:groupId,messages:[{type:'text',text:String(message).slice(0,4900)}]}),
    muteHttpExceptions:true
  });
  var code = res.getResponseCode();
  if (code < 200 || code >= 300) throw new Error('LINE API HTTP '+code+'：'+res.getContentText());
}
