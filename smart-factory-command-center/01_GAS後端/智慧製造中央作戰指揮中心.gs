 ...
 *
 * 重要：照片來源以 06_照片資料庫 為準，對應主鍵對照工號 / 產品編號 / 客戶品號 / 機台編號。
 */
...
function doPost(e) {
  ...
  if (p && p.events && Array.isArray(p.events)) {
    ...
    // LINE 製造 AI 助理 Read Only：沿用既有身份與角色權限，只讀 Gateway 正式資料。
    if (typeof LINE製造AI助理35_嘗試處理Webhook_ === 'function') {
      var 製造AI助理結果 = LINE製造AI助理35_嘗試處理Webhook_(p);
      if (製造AI助理結果 && 製造AI助理結果.已處理) {
        if (LINE重送守門 && !(製造AI助理結果.error || 製造AI助理結果.success === false)) {
          LINE訊息重送防護_完成_(LINE重送守門);
        }
        return 主程式_安全輸出JSON_(製造AI助理結果);
      }
    }
  ...
  var handlers = ['製造AI閘道_嘗試處理動作_', '主管戰情入口_嘗試處理動作_', '主管戰情看板_嘗試處理動作_', '每日自動化_嘗試處理動作_', 'LINE每日戰情推播_嘗試處理動作_', 'AI戰情資料源_嘗試處理動作_', '派班報工日結_嘗試處理動作_', '派班報工巡檢_嘗試處理動作_', '派班報工防呆_嘗試處理動作_', '今日派班報工_嘗試處理動作_', '排程需求池_嘗試處理動作_'];
...
function 轉Google圖片網址_(v) { const s = 文字_(v); if (!s) return ''; if (s.indexOf('data:image/') === 0) return s; const m = s.match(/[-\w]{25,}/); if (s.indexOf('drive.google.com') >= 0 && m) return 'https://drive.google.com/thumbnail?id=' + m[0] + '&sz=w800'; if (!/^https?:\/\//i.test(s) && m) return 'https://drive.google.com/thumbnail?id=' + m[0] + '&sz=w800'; return s; }
function 取照片資料_(obj, urlKeys, idKeys) { const url = 取值_(obj, urlKeys || []); let id = 取值_(obj, idKeys || []); if (!id) { const m = 文字_(url).match(/[-\w]{25,}/); if (m && 文字_(url).indexOf('drive.google.com') >= 0) id = m[0]; } const normalized = 轉Google圖片網址_(url || id); return { 照片網址: normalized, 縮圖網址: normalized, 檔案ID: id }; }
