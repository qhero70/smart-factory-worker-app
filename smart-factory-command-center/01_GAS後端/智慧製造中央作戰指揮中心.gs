/**
 * 智慧製造中央作戰指揮中心 正式主入口
 * 版本：正式完整版_v1.5.5｜GAS HTML 報工作業V2入口版
 ...
 * 3. Web App 可用 ?page=07_報工作業V2 開啟 GAS HTML 報工頁。
 * 4. API action=戰情 / AI摘要 / 主檔檢查 / 寫入報工作業v2_化新班別版 / 報工作業v2部署檢查 直接轉成主系統正式函數。
 */

const 系統設定 = {
  系統名稱: '智慧製造中央作戰指揮中心',
  版本: '正式完整版_v1.5.5_GAS_HTML報工作業V2入口版',
  時區: 'Asia/Taipei',
  主資料庫ID: '',
...
function doGet(e) {
  ...
  const 頁面 = 正規化HTML頁面名稱_(參數.page || 參數.頁面 || 參數.p || '');

  if (頁面) return 建立HTML頁面_(頁面);

  if (!動作) {
    return 建立HTML頁面_('index');
  }

  return 輸出JSON_(處理API請求_(動作, 參數));
}

function 正規化HTML頁面名稱_(page) {
...
function 處理API請求_(動作, 參數) {
  ...
  try {
    ...
    const map = {
      ...
      '寫入報工作業v2': () => typeof 寫入報工作業v2 === 'function' ? 寫入報工作業v2(p) : 寫入報工作業v2_化新班別版(p),
      '取得報工作業v2初始資料': () => typeof 取得報工作業v2初始資料 === 'function' ? 取得報工作業v2初始資料() : 取得主檔資料_(),
      '報工作業v2部署檢查': () => typeof 報工作業v2部署檢查_ === 'function' ? 報工作業v2部署檢查_() : 主入口健康檢查_(),
      '新增停機': () => 新增停機_(p),
      '新增換刀': () => 新增換刀_(p),
      '新增不良': () => 新增不良_(p),
      '取得戰情室': () => 取得戰情室_(),
...
function 報工作業v2部署檢查_() {
  ...
  return { 成功: 是否正常, 訊息: 是否正常 ? '報工作業v2部署檢查通過，可以送出報工' : '報工作業v2部署檢查未通過，請先初始化或修復欄位', 系統: 系統設定.系統名稱, 版本: 系統設定.版本, action: '報工作業v2部署檢查', 報工action: '寫入報工作業v2', 作業日: 取得作業日_(), 化新班別: 判斷化新班別_(), 不寫入資料: true, 表檢查 };
}
...
function 寫入報工作業v2_化新班別版(p) {
  ...
  const 結果 = 新增報工_(資料);
  return { 成功: true, 訊息: '報工作業v2化新班別版寫入完成', 後端函數: '寫入報工作業v2_化新班別版', 作業日: 資料.作業日, 班別: 資料.班別, 報工編號: 結果.資料['流水號'], 資料: 結果.資料 };
}

function 新增報工_(p) {
  const 資料 = { '流水號': p.報工編號 || 產生流水號_('RF'), '時間戳': new Date(), '作業日': p.作業日 || 取得作業日_(), '工號': p.工號 || '', '姓名': p.姓名 || '', '班別': p.班別 || '', '區域': p.區域 || '', '機台編號': p.機台編號 || p.主機台 || '', '產品編號': p.產品編號 || '', '品名': p.品名 || '', '工站名稱': p.工站名稱 || p.報工工站名稱 || '', '工單編號': p.工單編號 || '', '良品數': Number(p.良品數 || p.實際良品數 || 0), '不良數': Number(p.不良數 || 0), '停機分鐘': Number(p.停機分鐘 || 0), '停機原因': p.停機原因 || p.異常類型 || '', '換刀次數': Number(p.換刀次數 || 0), '備註': p.備註 || '', '來源': p.來源 || 'GAS_HTML_07_報工作業V2', '狀態': '有效' };
  新增資料列_('09_報工紀錄', 資料);
...
function 新增不良_(p) {
  const 資料 = { '流水號': 產生流水號_('NG'), '時間戳': new Date(), '作業日': p.作業日 || 取得作業日_(), '工號': p.工號 || '', '姓名': p.姓名 || '', '產品編號': p.產品編號 || '', '品名': p.品名 || '', '機台編號': p.機台編號 || '', '工單編號': p.工單編號 || '', '不良代碼': p.不良代碼 || '', '不良名稱': p.不良名稱 || p.不良原因 || '', '不良數量': Number(p.不良數量 || p.不良數 || 0), '責任歸屬': p.責任歸屬 || '', '說明': p.說明 || p.備註 || '', '照片網址': p.照片網址 || '' };
  新增資料列_('09_不良紀錄', 資料);
