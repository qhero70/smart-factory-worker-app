...
function doPost(e) {
  ...
  return 輸出JSON_(處理API請求_(內容.action || 內容.動作 || '', 內容));
}

function 處理前端請求(動作, 參數) { return 處理API請求_(動作, 參數 || {}); }
...
function 處理LINEWebhook_(內容) { (內容.events || []).forEach(ev => { if (ev.type === 'message' && ev.message.type === 'text') 回覆LINE_(ev.replyToken, 產生LINE回覆_(ev.message.text.trim())); }); return ContentService.createTextOutput('OK'); }
function 產生LINE回覆_(文字) { if (文字.includes('戰情') || 文字.includes('狀況')) { const x = 取得KPI戰情_(); return `📊 智慧製造戰情\n作業日：${x.作業日}\n總計畫：${x.KPI.總計畫}\n總完成：${x.KPI.總完成}\n達成率：${x.KPI.達成率}%\n不良率：${x.KPI.不良率}%\n停機：${x.KPI.今日停機分鐘} 分鐘\n逾期警示：${x.KPI.逾期工單數} 筆`; } if (文字.includes('AI')) { const x = 產生AI摘要_(); return `🤖 AI摘要\n風險：${x.風險等級}\n${x.摘要}\n建議：\n${x.建議.join('\n')}`; } return '🏭 智慧製造中央作戰指揮中心\n可輸入：戰情、AI摘要、工單、報工。'; }
function 回覆LINE_(replyToken, text) { if (!系統設定.LINE_CHANNEL_ACCESS_TOKEN) return; UrlFetchApp.fetch('https://api.line.me/v2/bot/message/reply', { method: 'post', contentType: 'application/json', headers: { Authorization: 'Bearer ' + 系統設定.LINE_CHANNEL_ACCESS_TOKEN }, payload: JSON.stringify({ replyToken, messages: [{ type: 'text', text }] }), muteHttpExceptions: true }); }
function 解析POST內容_(e) { if (!e || !e.postData || !e.postData.contents) return {}; try { return JSON.parse(e.postData.contents); } catch (err) { return e.parameter || {}; } }
...
function 記錄操作_(操作者, 動作, 目標, 結果, 備註) { try { 新增資料列_('系統_操作紀錄', { '時間戳': new Date(), '操作者': 操作者, '動作': 動作, '目標': 目標, '結果': 結果, '備註': 備註 }); } catch (e) {} }
function 寫入預設資料_() { const ss = 取得試算表_(); if (ss.getSheetByName('00_系統設定').getLastRow() < 2) [['系統名稱', 系統設定.系統名稱, '系統顯示名稱', new Date()], ['版本', 系統設定.版本, '目前版本', new Date()], ['低達成率警戒', '80', '低於此值顯示警示', new Date()]].forEach(r => ss.getSheetByName('00_系統設定').appendRow(r)); if (ss.getSheetByName('05_共用資料').getLastRow() < 2) [['班別','早班','早班',1,'是',''],['班別','中班','中班',2,'是',''],['班別','夜班','夜班',3,'是',''],['停機原因','換線','換線',1,'是',''],['停機原因','待料','待料',2,'是',''],['停機原因','設備異常','設備異常',3,'是',''],['不良類別','加工不良','加工不良',1,'是',''],['不良類別','素材不良','素材不良',2,'是','']].forEach(r => ss.getSheetByName('05_共用資料').appendRow(r)); if (ss.getSheetByName('LINE_指令設定').getLastRow() < 2) [['戰情','取得今日戰情','文字','12_AI分析中心','是',1,''],['AI摘要','取得AI摘要','文字','12_AI分析中心','是',2,''],['報工','開啟報工入口','連結','09_報工系統','是',3,'']].forEach(r => ss.getSheetByName('LINE_指令設定').appendRow(r)); }
