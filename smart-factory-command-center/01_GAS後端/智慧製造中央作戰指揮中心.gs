/**
 * 智慧製造中央作戰指揮中心｜正式主後端
 * 版本：v1.6.0｜乾淨正式版｜報工作業V2整併版
 *
 * 使用規則：
 * 1. 本檔是唯一正式 doGet / doPost 入口。
 * 2. 本檔已內建報工作業V2後端：取得報工作業v2初始資料、寫入報工作業v2。
 * 3. 不需要再貼任何 ZZ_、補丁、修正版、測試版 .gs。
 * 4. 報工正式寫入分頁：09_報工。
 * 5. 前端正式 HTML：07_報工作業V2.html。
 */

const 系統設定 = {
  系統名稱: '智慧製造中央作戰指揮中心',
  版本: 'v1.6.0_乾淨正式版_報工作業V2整併版',
  時區: 'Asia/Taipei',
  主資料庫ID: ''
};
...
function doGet(e) {
  e = e || { parameter: {} };
  const p = e.parameter || {};
  const page = String(p.page || p.頁面 || p.p || '').trim();
  const action = String(p.action || p.動作 || '').trim();
...
function doPost(e) {
  const p = 解析POST_(e);
  if (p && p.events && Array.isArray(p.events)) return ContentService.createTextOutput('OK');
...
function 健康檢查() {
  return { 成功: true, 系統: 系統設定.系統名稱, 版本: 系統設定.版本, 時間: new Date(), 入口: 'v1.6.0_clean' };
}
...
function 建立報工工站群組_(產品, 工站, 機台) {
  ...
  return 工站.map(g => {
    ...
    const p = 產品Map[產品編號] || {};
    const 機台字串 = 取值_(g, ['機台清單', '機台編號清單', '機台編號', '主機台']);
    const 機台清單 = 拆字串_(機台字串).map(id => {
    ...
    return {
      ...
      顯示名稱: [工站名稱, 工序, 機台清單.map(m => m.機台編號).join('、')].filter(Boolean).join('｜')
    };
...
function 同步不良紀錄_(obj, data) {
  ...
  const ss = 取得試算表_();
  const sh = 建立或修復表_(ss, '09_不良紀錄', 正式工作表規格['09_不良紀錄']);
  const headers = 取表頭_(sh);
  const row = {
    流水號: 產生流水號_('NG'), 時間戳: new Date(), 作業日: obj.作業日, 工號: obj.工號, 姓名: obj.姓名,
    產品編號: obj.產品編號, 品名: obj.品名, 機台編號: obj.主機台 || obj.機台清單, 工單編號: data.工單編號 || '',
    不良代碼: obj.不良代碼, 不良名稱: obj.不良原因, 不良數量: obj.不良數, 責任歸屬: obj.不良類別, 說明: obj.備註, 照片網址: ''
  };
...
function 記錄操作_(操作者, 動作, 目標, 結果, 備註) {
  try {
    const sh = 建立或修復表_(取得試算表_(), '系統_操作紀錄', 正式工作表規格['系統_操作紀錄']);
    sh.appendRow([new Date(), 操作者, 動作, 目標, 結果, 備註 || '']);
  } catch (e) {}
}

function 取得作業日_() {
  const now = new Date();
  const hh = Number(Utilities.formatDate(now, 系統設定.時區, 'H'));
  const mm = Number(Utilities.formatDate(now, 系統設定.時區, 'm'));
  const d = new Date(now);
  if (hh < 6 || (hh === 6 && mm < 10)) d.setDate(d.getDate() - 1);
  return Utilities.formatDate(d, 系統設定.時區, 'yyyy-MM-dd');
}

function 判斷化新班別_() {
  const hm = Number(Utilities.formatDate(new Date(), 系統設定.時區, 'HHmm'));
  if (hm >= 750 && hm < 1650) return '早班';
  if (hm >= 1650 || hm < 110) return '中班';
  return '大夜班';
}

function 產生流水號_(prefix) {
  return prefix + '-' + Utilities.formatDate(new Date(), 系統設定.時區, 'yyyyMMddHHmmss') + '-' + Math.floor(Math.random() * 900 + 100);
}
...
function 取共用清單_(rows, type, defaults) {
  const list = rows.filter(x => 文字_(x.資料類型) === type && 文字_(x.啟用 || '是') !== '否').sort((a, b) => Number(a.排序 || 999) - Number(b.排序 || 999)).map(x => ({ 名稱: x.名稱 || x.代碼, 值: x.名稱 || x.代碼 }));
  return list.length ? list : defaults.map(x => ({ 名稱: x, 值: x }));
}

function 取共用值清單_(rows, type, defaults) {
  const list = 取共用清單_(rows, type, defaults).map(x => x.值 || x.名稱);
  return list.length ? list : defaults;
}

function 取不良原因_(rows) {
  const z = [], y = [];
  rows.forEach(x => {
    const code = 文字_(x.代碼);
    const name = 文字_(x.名稱 || x.代碼);
    if (!code && !name) return;
    if (code.toUpperCase().startsWith('Z')) z.push({ 代碼: code, 名稱: name });
    if (code.toUpperCase().startsWith('Y')) y.push({ 代碼: code, 名稱: name });
  });
  return { Z: z.length ? z : [{ 代碼: 'Z01', 名稱: '素材 / 外觀不良' }], Y: y.length ? y : [{ 代碼: 'Y01', 名稱: '加工 / 尺寸不良' }] };
}
