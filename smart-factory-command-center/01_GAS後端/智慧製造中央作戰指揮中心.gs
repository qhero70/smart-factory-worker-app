...
function 測試_健康檢查() { return 處理API請求_('健康檢查', {}); }
function 測試_建立測試工單() { return 新增工單_({ 工單編號: 'MO-TEST-001', 產品編號: 'A900200000', 品名: '測試產品', 計畫量: 100, 優先級: '高', 狀態: '待生產' }); }
...
function 取得主檔資料_() {
  return { 成功: true, 人員: 表格轉物件_('01_人員主檔'), 產品: 表格轉物件_('02_產品主檔'), 機台: 表格轉物件_('03_機台主檔'), 工站: 表格轉物件_('04_工站主檔'), 共用: 表格轉物件_('05_共用資料') };
}
...
function 新增工單_(p) {
  const 資料 = { '工單編號': p.工單編號 || 產生流水號_('MO'), '產品編號': p.產品編號 || '', '品名': p.品名 || '', '計畫量': Number(p.計畫量 || 0), '已完成量': Number(p.已完成量 || 0), '不良量': Number(p.不良量 || 0), '開始日': p.開始日 || 取得作業日_(), '預計完工日': p.預計完工日 || '', '優先級': p.優先級 || '一般', '狀態': p.狀態 || '待生產', '備註': p.備註 || '', '更新時間': new Date() };
  新增資料列_('10_工單主檔', 資料);
...
function 更新工單完成量_(工單編號, 良品數, 不良數) {
  ...
  const idx工單 = headers.indexOf('工單編號'), idx完成 = headers.indexOf('已完成量'), idx不良 = headers.indexOf('不良量'), idx計畫 = headers.indexOf('計畫量'), idx狀態 = headers.indexOf('狀態'), idx更新 = headers.indexOf('更新時間');
...
function 取得KPI戰情_() {
  ...
  const 工單 = 表格轉物件_('10_工單主檔');
  const 報工 = 表格轉物件_('09_報工紀錄');
  const 停機 = 表格轉物件_('09_停機紀錄');
  const 不良 = 表格轉物件_('09_不良紀錄');
  const 今日報工 = 報工.filter(x => String(x['作業日']) === 今日);
  const 今日停機 = 停機.filter(x => String(x['作業日']) === 今日);
  const 今日不良明細 = 不良.filter(x => String(x['作業日']) === 今日);
  const 總計畫 = 工單.reduce((s, x) => s + Number(x['計畫量'] || 0), 0);
  const 總完成 = 工單.reduce((s, x) => s + Number(x['已完成量'] || 0), 0);
  const 今日良品 = 今日報工.reduce((s, x) => s + Number(x['良品數'] || 0), 0);
  const 今日不良 = 今日報工.reduce((s, x) => s + Number(x['不良數'] || 0), 0) + 今日不良明細.reduce((s, x) => s + Number(x['不良數量'] || 0), 0);
  const 今日停機分鐘 = 今日報工.reduce((s, x) => s + Number(x['停機分鐘'] || 0), 0) + 今日停機.reduce((s, x) => s + Number(x['停機分鐘'] || 0), 0);
  const 達成率 = 總計畫 ? Math.round(總完成 / 總計畫 * 1000) / 10 : 0;
  const 不良率 = (今日良品 + 今日不良) ? Math.round(今日不良 / (今日良品 + 今日不良) * 1000) / 10 : 0;
...
function 產生異常警示_() {
  ...
  return 工單.filter(x => { if (!x['預計完工日']) return false; const due = new Date(x['預計完工日']); return due < today && String(x['狀態']) !== '已完成'; }).map(x => ({ 類型: '交期風險', 工單編號: x['工單編號'], 品名: x['品名'], 訊息: '已超過預計完工日仍未完成' }));
}

function 產生AI摘要_() {
  const 戰情 = 取得戰情室_();
  const KPI = 取得KPI戰情_();
  const 風險 = 戰情.警示.length > 0 ? '高' : (戰情.達成率 < 60 || KPI.KPI.今日停機分鐘 > 120 ? '中' : '低');
  const 建議 = [];
  if (戰情.達成率 < 80) 建議.push('今日需優先確認進行中工單與瓶頸機台。');
  if (KPI.KPI.今日停機分鐘 > 120) 建議.push('今日停機分鐘偏高，請先查看停機排行與設備狀態。');
  if (KPI.KPI.不良率 > 3) 建議.push('今日不良率偏高，請追蹤不良排行與機台/工站關聯。');
  if (戰情.警示.length) 建議.push('請先處理逾期工單，避免交期風險擴大。');
  if (!建議.length) 建議.push('目前狀態穩定，建議持續監控報工即時性。');
  const 結果 = { 成功: true, 風險等級: 風險, 摘要: `目前總達成率 ${戰情.達成率}%，今日不良率 ${KPI.KPI.不良率}%，今日停機 ${KPI.KPI.今日停機分鐘} 分鐘。`, 建議 };
  新增資料列_('12_AI分析紀錄', { '流水號': 產生流水號_('AI'), '時間戳': new Date(), '分析類型': '戰情摘要', '輸入摘要': JSON.stringify(KPI.KPI), '分析結果': 結果.摘要, '建議動作': 建議.join('\n'), '風險等級': 風險, '是否完成': '否' });
  return 結果;
}
...
function 匯入內建測試資料_(p) {
  ...
  記錄操作_('系統維護', '匯入測試資料', '內建測試資料', '完成', 覆蓋 ? '覆蓋' : '空表才匯入');
  return { 成功: true, 訊息: '內建測試資料匯入完成', 覆蓋 };
}

function 處理LINEWebhook_(內容) {
  const events = 內容.events || [];
  events.forEach(ev => { if (ev.type === 'message' && ev.message.type === 'text') { const text = ev.message.text.trim(); const replyToken = ev.replyToken; const msg = 產生LINE回覆_(text); 回覆LINE_(replyToken, msg); } });
  return ContentService.createTextOutput('OK');
}

function 產生LINE回覆_(文字) {
  if (文字.includes('戰情') || 文字.includes('狀況')) { const x = 取得KPI戰情_(); return `📊 智慧製造戰情\n作業日：${x.作業日}\n總計畫：${x.KPI.總計畫}\n總完成：${x.KPI.總完成}\n達成率：${x.KPI.達成率}%\n不良率：${x.KPI.不良率}%\n停機：${x.KPI.今日停機分鐘} 分鐘\n逾期警示：${x.KPI.逾期工單數} 筆`; }
  if (文字.includes('AI')) { const x = 產生AI摘要_(); return `🤖 AI摘要\n風險：${x.風險等級}\n${x.摘要}\n建議：\n${x.建議.join('\n')}`; }
  return '🏭 智慧製造中央作戰指揮中心\n可輸入：戰情、AI摘要、工單、報工。';
}
