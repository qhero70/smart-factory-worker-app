...
"""
化新精密有限公司｜智慧 5S 主管操作手冊 Word 產生器

以受控 Markdown 建立正式 Word，沿用現場手冊的緊湊參考指南版型，
並針對管理役割、戰情、驗證、紅牌、逾期與會議閉環進行結構稽核。
"""
...
來源檔案 = 目前目錄 / "智慧5S主管操作手冊_第1.0版.md"
輸出檔案 = 目前目錄 / "智慧5S主管操作手冊_第1.0版.docx"
稽核檔案 = 目前目錄 / "智慧5S主管操作手冊_第1.0版_結構稽核.json"
...
def 選擇主管欄寬(表頭: 序列型別[str]) -> 清單型別[int]:
    """以 9360 DXA 為固定網格，讓管理判定與說明欄保有足夠閱讀寬度。"""
    欄數 = len(表頭)
    合併表頭 = "|".join(表頭)
    ...
    if 欄數 == 1:
        return [9360]
    if 欄數 == 2:
        if any(文字 in 合併表頭 for 文字 in ("正確用途", "完整寫法")):
            return [4680, 4680]
        return [2400, 6960]
    if 欄數 == 3:
        if "分數" in 合併表頭:
            return [1300, 3000, 5060]
        if "管理判定" in 合併表頭 or "主管動作" in 合併表頭:
    ...
    if 欄數 == 4:
        ...
        if "顯示角色" in 合併表頭:
            return [1450, 2850, 2500, 2560]
        if "功能" in 合併表頭 and "按鈕" in 合併表頭:
            return [1500, 3100, 1800, 2960]
        if "原狀態" in 合併表頭:
            return [1300, 1900, 2900, 3260]
        return [1500, 2750, 2400, 2710]
    return 現場手冊工具.選擇現場欄寬(表頭)


# 保留緊湊版型、真實清單與小表格不拆分規則，只依管理內容調整欄寬。
規格.選擇欄寬 = 選擇主管欄寬


def 設定主管頁首頁尾(區段) -> None:
    頁首 = 區段.header.paragraphs[0]
    規格.清除段落(頁首)
    頁首.alignment = 段落對齊.CENTER
    頁首.paragraph_format.space_after = 點(0)
    執行文字 = 頁首.add_run("化新精密有限公司｜智慧 5S 主管操作手冊｜第 1.0 版")
    ...
    for 頁尾物件 in (區段.footer, 區段.first_page_footer):
        ...
        執行文字 = 頁尾.add_run("化新精密有限公司｜智慧 5S 主管操作手冊｜第 ")
        規格.設定執行文字(執行文字, 大小=8.5, 顏色=規格.灰字)
        規格.加入頁碼欄位(頁尾)
        執行文字 = 頁尾.add_run(" 頁")
        規格.設定執行文字(執行文字, 大小=8.5, 顏色=規格.灰字)
...
def 新增主管封面(doc: 文件, 前言: 序列型別[str]) -> None:
    ...
    執行文字 = 副標題.add_run("戰情、風險、驗證、紅牌、逾期與會議管理，逐案形成閉環")
    規格.設定執行文字(執行文字, 大小=13, 顏色=規格.灰字)

    封面資料 = [
        ["管制項目", "內容", "管制項目", "內容"],
        ["文件編號", 文件資料.get("文件編號", "化新－5S－主管－001"), "文件版本", 文件資料.get("文件版本", "第 1.0 版")],
        ["對應系統", 文件資料.get("對應系統", "智慧 5S 管理平台第 1.0.2 版"), "文件狀態", 文件資料.get("文件狀態", "正式版（核准後生效）")],
        ["制定單位", 文件資料.get("制定單位", "製造部"), "制定日期", 文件資料.get("制定日期", "2026 年 8 月 10 日")],
    ]
    ...
    執行文字 = 提醒.add_run("管理口訣：先看風險、再抓責任、到場驗證、依證據結案、收班同步歸零。")
...
def 建立文件() -> 文件:
    ...
    doc = 文件()
    doc.core_properties.title = "化新精密有限公司｜智慧 5S 主管操作手冊"
    doc.core_properties.subject = "智慧 5S 管理平台第 1.0.2 版主管與區域負責人管理指南"
    doc.core_properties.author = "化新精密有限公司"
    doc.core_properties.last_modified_by = "化新精密有限公司"
    doc.core_properties.keywords = "智慧5S,主管,戰情,改善驗證,紅牌,逾期,同步,會議"
...
def 稽核主管手冊(檔案: 路徑) -> dict:
    ...
    for 文字 in 必備文字:
        if 文字 not in 全文:
            報告["結構錯誤"].append(f"缺少主管手冊基準：{文字}")
    if 報告.get("表格數", 0) < 25:
        報告["結構錯誤"].append("主管手冊表格數不足 25，疑似內容未完整輸出")
    if 報告.get("真實編號定義數", 0) < 20:
        報告["結構錯誤"].append("主管手冊真實編號定義不足 20，疑似步驟未完整輸出")
