...
const 角色分流程式 = 路徑工具.join(後端目錄, '34_LINE_使用者角色分流與一般員工選單.gs');
...
驗收('語法', 'LINE 入口、Rich Menu、群組橋接與排名模組可解析', () => {
  [正式入口程式, 快捷選單程式, 角色分流程式, 群組橋接程式, 排名程式, 主程式, 最終接線程式]
...
驗收('圖片', '主管與員工正式圖皆為 1200×810 PNG 且小於 1MB', () => {
  ['richmenu-supervisor-v190.png', 'richmenu-worker-v186.png'].forEach(檔名 => {
...
驗收('角色分流', '嘉欣工程師權限60固定使用主管入口，一般員工才使用員工入口', () => {
  const 上下文 = 載入(角色分流程式, {});
  const 嘉欣 = { 工號: 'fhfi573', 姓名: '黃嘉欣', 職稱: '工程師', 角色: '工程師', 權限等級: 60, 允許主管入口: '是', 啟用: '是' };
  const 一般員工 = { 工號: 'TEST001', 姓名: '測試人員', 職稱: '作業員', 角色: '現場人員', 權限等級: 10, 允許主管入口: '否', 啟用: '是' };
  斷言.equal(上下文.LINE角色分流34_是否主管選單_(嘉欣), true, '嘉欣不得被降為一般員工入口');
  斷言.equal(上下文.LINE角色分流34_是否主管選單_(一般員工), false, '一般員工不得誤用主管入口');
  const 程式 = 讀取(角色分流程式);
  斷言.ok(程式.includes("目標選單 = LINE角色分流34_是否主管選單_(身份) ? '主管入口' : '一般員工入口'"), '批次同步必須依身份分流');
});
